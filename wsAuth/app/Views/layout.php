                ...
                <?php if (isset($_SESSION['user'])): ?>
                    <div class="relative group">
                        <button class="flex items-center space-x-2 bg-black/20 px-4 py-2 rounded-full border border-white/10 hover:bg-black/30 transition">
                            ...
                            <span class="font-bold text-sm"><?php echo htmlspecialchars($_SESSION['user']['name']); ?></span>
                        </button>
                        <div class="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl py-2 hidden group-hover:block text-gray-800 border border-gray-100 overflow-hidden">
                            <div class="px-4 py-2 bg-gray-50 border-b border-gray-100 mb-1">
                                ...
                                <p class="text-xs font-bold text-[#1a4731]"><?php echo htmlspecialchars(strtoupper($_SESSION['user']['role'])); ?></p>
                            </div>
    ...
    <?php if (isset($_SESSION['user'])): ?>
        <script>
            window.setTimeout(function () {
                ...
                const redirectAction = params.get('action') || 'home';
                window.location.href = 'index.php?action=session_expired&redirect=' + encodeURIComponent(redirectAction);
            }, <?php echo \App\Support\SessionManager::getLifetimeSeconds() * 1000; ?>);
        </script>
    <?php endif; ?>
</body>
</html>
